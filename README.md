...
### 7.3 バリデーションとストレージ方針
- `POST /api/videos` は `multipart/form-data` を受け付け、`prompt`/`model`/`size`/`seconds` に加えて任意の `input_reference` ファイルを同時送信する。
- `input_reference` は 20MB までの画像または動画ファイルを想定し、超過した場合はリクエストを 400 応答で拒否する。
- 参照ファイルは OpenAI API へストリーミング転送するためだけにメモリ上で保持し、動画レコードや永続ストレージには保存しない。転送後は破棄し、サーバーディスクにも書き込まない。
