...
## 13. 実装済み MVP のセットアップ方法

このリポジトリには上記要件を満たす最小構成の Node.js 製バックエンドと、素の HTML/CSS/JavaScript によるシングルページ UI を含んでいます。以下の手順でローカル実行できます。

1. 環境変数に OpenAI の API キーを設定する

   ```bash
   export OPENAI_API_KEY="sk-..."
   ```

2. サーバを起動する（依存ライブラリは使用していません）
...
| メソッド | パス | 説明 |
| --- | --- | --- |
| `POST` | `/api/videos` | OpenAI Videos API に生成ジョブを作成し、自前 ID を発行 |
| `GET` | `/api/videos` | 生成ジョブの一覧を取得 |
| `GET` | `/api/videos/:id/status` | 個別ジョブの最新ステータスを返す（サーバ内キャッシュを返却）|
| `GET` | `/api/videos/:id/content` | OpenAI から動画バイナリをストリーミング取得（Range 対応）|
| `GET` | `/api/settings` | API キー設定済みかのフラグを返す |

バックエンドは OpenAI から受け取った `providerVideoId` を基に 5 秒間隔で進行状況をポーリングし、完了後にフロントエンドから `<video>` タグ経由で再生できるようにコンテンツをプロキシ配信します。
